...
export class AddActivitySeriesId1747197000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "activities" ADD COLUMN IF NOT EXISTS "series_id" varchar DEFAULT NULL`,
    );
    await queryRunner.query(
    ...
    `);
    const colNames: string[] = cols.map(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "activities" DROP COLUMN IF EXISTS "series_id"`,
    );
  }
}
