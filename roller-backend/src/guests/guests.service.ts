...
export class GuestsService {
  ...
  async exportAll(
    ...
    const groups = await this.groupsRepository.find({
      select: ['id', 'group_code', 'car_count'],
    });
    const regionMap = new Map(regions.map((r) => [r.id, r.name]));
    const groupMap = new Map(groups.map((g) => [g.id, g.group_code]));
    const groupCarCountMap = new Map(groups.map((g) => [g.id, g.car_count]));

    return this.buildGuestsExcel(guests, regionMap, groupMap, groupCarCountMap);
  }
  ...
  private buildGuestsExcel(
    ...
    groupCarCountMap = new Map<string, number | null>(),
  ): Buffer {
    const headers = [
      'guest_code',
      'full_name',
    ...
    const rows = guests.map((g) => [
      ...
      g.is_minor ? 'Sí' : 'No',
      g.branch ?? '',
      g.is_group_contact ? 'Sí' : 'No',
      g.native_language ?? '',
      (g.other_languages ?? []).join(', '),
      g.speaks_english ? 'Sí' : 'No',
      g.is_special_servant ? 'Sí' : 'No',
      g.origin_city ?? '',
      g.email ?? '',
      g.available_from ?? '',
      g.available_to ?? '',
      g.arrival_transport ?? '',
      g.arrival_other_transport ?? '',
      g.arrival_date ?? '',
      g.arrival_time ?? '',
      g.arrival_place ?? '',
      g.arrival_airport ?? '',
      ...
      g.checkin_date ?? '',
      g.checkout_date ?? '',
      g.needs_special_accommodation ? 'Sí' : 'No',
      g.hosting_address ?? '',
      g.maps_link ?? '',
      g.lat ?? '',
      g.lng ?? '',
      g.transport_mode ?? '',
      g.car_seats ?? '',
      groupCarCountMap.get(g.group_id) ?? '',
      g.terms_accepted ? 'Sí' : 'No',
      g.terms_accepted_at ?? '',
      g.terms_version ?? '',
      g.created_at instanceof Date ? g.created_at.toISOString() : g.created_at,
      g.updated_at instanceof Date ? g.updated_at.toISOString() : g.updated_at,
    ]);
    const wb = XLSX.utils.book_new();
